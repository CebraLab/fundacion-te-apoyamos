 ...
 */
const apiProxyTarget = (process.env.API_BASE_URL || 'http://rmq_app_queue:3000').replace(/\/$/, '')

const createApiProxy = () => createProxyMiddleware({
  target: apiProxyTarget,
  changeOrigin: true,
  pathRewrite(path, req) {
    const rewritten = `/api/v1${path}`
    console.log(`[proxy][rewrite] ${req.method} ${path} -> ${rewritten}`)
    return rewritten
  },
  onProxyReq(proxyReq, req) {
    const forwardedPath = proxyReq.path ?? ''
    console.log(`[proxy][request] ${req.method} ${req.originalUrl} -> ${apiProxyTarget}${forwardedPath}`)
  },
...
const apiProxyMiddleware = createApiProxy()
...
export const create = defineSsrCreate((/* { ... } */) => {
  ...
  app.use((req, res, next) => {
    console.log(`[ssr][incoming] ${req.method} ${req.originalUrl}`)
    next()
  })
